...
-module(avl_tree).
-export([new/0,push/3,inTree/2, remove/2, rotate/1, depth/1, balance_tree/1, search/2]).
...
search({}, _) -> {error,not_found};
search({Key, Value, _, _,_}, Key) -> {Key, Value};
search({Key, _, Left, _, _}, AKey) when AKey < Key -> search(Left, AKey);
search({_, _, _, Right, _}, AKey) -> search(Right, AKey).
